...
! ***********************************************************************
      ...
      subroutine extras_controls(id, ierr)
         ...
         s% other_alpha_mlt => new_other_alpha_mlt

      end subroutine extras_controls
      
      subroutine new_other_alpha_mlt(id, ierr)
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         type (star_info), pointer :: s
         integer :: k
         ...
         if (ierr /= 0) return
        do k = 1 , s% nz
            s% alpha_mlt(k) = ((s%x_ctrl(8)-s%x_ctrl(7))/2.0d0)* tanh((s%m(k)/Msun - s%x_ctrl(5))/s%x_ctrl(6))+((s%x_ctrl(8)+s%x_ctrl(7))/2.0d0)
        end do      
      end subroutine new_other_alpha_mlt
      ...
      integer function extras_finish_step(id)
        ...
        !
        !save a profile ,
        !s % n e e d _ t o_ s a v e _ p r o f il e s _ n o w = . true .
        !update the star log ,
        !s % n e e d _ t o _ u p d a t e _ h i s t o r y _ n o w = . true .
